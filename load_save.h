...
{
	// read the matrix from a binary file byte by byte
	for (int i = 0; i < p->height; i++)
		{
			for (int j = 0; j < p->width; j++) {
				for (int k = 0; k < p->pixel_no; k++) {
...
image load_image(FILE **f, char *name)
{
	...
	skip_com(f, &p);
	fseek(*f, strlen(p) * (-1), SEEK_CUR);
	//move the coursor pointer backwards again to read save the scale as an int
	if (scale > 0) {
	fscanf(*f, "%d", &result.scale);
	fread(&c, 1, 1, *f);
	fseek(*f, -1, SEEK_CUR);
	// check if there are any comments between the scale and the number
	// if so, skip them, else read the matrix
	if (c == '#') {
	fscanf(*f, "%s", p);
	skip_com(f, &p);
		fseek(*f, strlen(p) * (-1), SEEK_CUR);
...
{
	// get the string corresponding to the image type
	// (P1, P2, ..., P6)
